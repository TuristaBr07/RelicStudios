...
function BauMensalPage({ onSubscribe }) {
  const tiers = [
    {
      ...
      lootline: "1 figure série rara + brindes",
      tier: "comum",
      perks: [
        "1 figure de tier RARO por mês",
        "Pin colecionável da edição",
        "Frete grátis Brasil",
        "Acesso ao Discord da Taverna",
      ],
    },
    {
      id: "capitao",
      ...
      lootline: "1 figure épica + 2 brindes premium",
      tier: "epico",
      featured: true,
      perks: [
        "1 figure de tier ÉPICO por mês",
        "2 brindes premium curados",
        "Acesso antecipado a drops (24h)",
        "Frete grátis Brasil + 10% off na loja",
        "Pin + certificado numerado",
      ],
    },
    {
      id: "lenda",
      ...
      lootline: "1 figure lendária + tudo do Capitão",
      tier: "lendaria",
      perks: [
        "1 figure de tier LENDÁRIA por mês",
        "Edição numerada exclusiva (<50 unidades)",
        "Acesso antecipado a drops (72h)",
        "Frete grátis worldwide",
        "20% off na loja + 1 brinde anual",
      ],
    },
  ];
  ...
  return (
    <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, marginTop: 40 }}>
            {[
              ...
              <div key={s.n} style={{
                ...
                <img src={`assets/icons/${s.glyph}.svg`} style={{ width: 40, height: 40, marginBottom: 16 }} />
                <h3 style={{
                  fontFamily: '"Bebas Neue", Impact, sans-serif',
      ...
      {/* Sneak peek */}
      <section style={{ padding: "80px 0", background: "var(--vault-800)", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div className="container">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginTop: 32 }}>
            {[
              { tag: "ANIMES", img: "assets/products/cc-code-geass.jpg", label: "1 figure lendária" },
              { tag: "GAMES",  img: "assets/products/dante-dmc.jpg",     label: "Tier épico ou superior" },
              { tag: "BRINDE", img: null, glyph: "scroll", label: "Certificado numerado" },
              { tag: "BRINDE", img: null, glyph: "coin",   label: "Moeda colecionável" },
            ].map((it, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  background: it.img ? "#0E1219" : "linear-gradient(180deg, #2A323F 0%, #0E1219 100%)",
                  border: "1px solid var(--pewter-400)",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  overflow: "hidden", position: "relative",
                }}>
                  {it.img
                    ? <img src={it.img} style={{ width: "100%", height: "100%", objectFit: "cover", filter: "blur(2px) brightness(0.6)" }}/>
                    : <img src={`assets/icons/${it.glyph}.svg`} style={{ width: 56, height: 56, opacity: 0.5 }}/>}
                  <div style={{
                    position: "absolute", inset: 0,
      ...
      <section style={{ padding: "0 0 96px" }}>
        <div className="container">
          <div style={{
            ...
            <img src="assets/icons/chest.svg" style={{ width: 56, height: 56, marginBottom: 18, opacity: 0.85 }}/>
            <h2 style={{
              fontFamily: '"Bebas Neue", Impact, sans-serif',
  ...
  return (
    <div style={{
      ...
      background: tier.featured
        ? "linear-gradient(180deg, #2C2218 0%, #1F1812 100%)"
        : "var(--vault-800)",
      ...
      {tier.featured && (
        <div style={{
          ...
          color: "#1A0B02",
          boxShadow: "0 0 20px -4px rgba(232,176,40,0.6)",
          whiteSpace: "nowrap",
        }}>MAIS ESCOLHIDO</div>
      )}
      <div>
      ...
      <div style={{ padding: "14px 0", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--fg-faint)" }}>R$</span>
          <span style={{
            fontFamily: '"Bebas Neue", Impact, sans-serif',
            fontSize: 56, color: "var(--gold-200)", lineHeight: 1,
          }}>{tier.price}</span>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--fg-faint)" }}>/mês</span>
        </div>
        <div style={{
          fontFamily: "var(--font-body)", fontSize: 12, fontWeight: 500,
          color: "var(--gold-300)", marginTop: 4,
        }}>{tier.lootline}</div>
      </div>
      <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
        {tier.perks.map(p => (
          <li key={p} style={{
            ...
            <span style={{ flexShrink: 0, color: "var(--gold-300)", marginTop: 4 }}>
      ...
      >ESCOLHER {tier.name.toUpperCase()}</Button>
    </div>
  );
}
  ...
  return (
    <div style={{ background: "var(--vault-700)" }}>
      ...
      {open && (
        <div style={{
          padding: "0 4px 22px",
          fontFamily: "var(--font-body)", fontSize: 14, color: "var(--fg-muted)", lineHeight: 1.65,
        }}>{a}</div>
      )}
    </div>
  );
